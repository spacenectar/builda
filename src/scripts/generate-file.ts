...
import getCSSExt from './get-css-ext';
...
interface Props {
  ...
  componentNameSentenceCase: string;
...
const generateFile = (name: string, props: Props) => {
  const {
    componentDir,
...
`;
  ...
  const stylesheet =
    (preprocessor && getCSSExt(preprocessor, useModules)) || '';

  const srcName = (name: string) => {
    if (name === 'styles') {
  ...
  const scaffoldPath = prepopulate
    ? path.join(appDir, 'scaffold', 'prepopulated')
    : path.join(appDir, 'scaffold', 'blank');

  const cssString = useModules
    ? `// Import Stylesheet \nimport styles from './styles.${stylesheet}'`
    : `import './styles.${stylesheet}'`;
  const classesString = useModules
    ? `styles['${componentNameKebab}']`
    : `"style-${componentNameKebab}"`;
  ...
  const extraParams = storyParams
    ? makeStoryParams(use_mdx, readme, storyParams)
    : '';

  const replaceText = () => {
    return fs.readFile(
      `${scaffoldPath}/${srcName(name)}`,
      'utf8',
      (err, data) => {
        if (err)
        ...
        if (name === 'index.stories.tsx' || name === 'index.stories.jsx') {
          newData = newData.replace(/%readmeimport%/g, importedReadme);
        }

        storyParams &&
          (newData = newData.replace(/%storyParams%/g, extraParams));
