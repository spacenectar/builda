...
describe('convertRegistryPathToUrl() function', () => {
  ...
  test('should return a path to a registry.json file on the builda repository when builda: is provided', () => {
    ...
    expect(convertRegistryPathToUrl({ registryPath, config })).toEqual(
      expected
    );
  });
  ...
  test('should return a raw path to a registry.json file along with the correct type when a github resolver is provided along with `withType`', () => {
    ...
    expect(
      convertRegistryPathToUrl({ registryPath, config, withType: true })
    ).toEqual(expected);
  });
  ...
  test('Should return a path to a registry.json when a custom matcher is provided', () => {
    ...
    expect(
      convertRegistryPathToUrl({ registryPath, config: customConfig })
    ).toEqual(expected);
  });
