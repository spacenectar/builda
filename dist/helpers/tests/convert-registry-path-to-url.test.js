...
describe('convertRegistryPathToUrl() function', () => {
    ...
    test('should return a raw path to a registry.json file when a github resolver is provided', () => {
        const registryPath = 'github:test-path/builda@latest';
        const expected = 'https://raw.githubusercontent.com/test-path/builda/latest';
        expect((0, convert_registry_path_to_url_1.convertRegistryPathToUrl)({ registryPath, config })).toEqual(expected);
    });
    test('should return a raw path to a registry.json file along with the correct type when a github resolver is provided along with `withType`', () => {
        const registryPath = 'github:test-path/builda@latest';
        const expected = {
            type: 'github',
            url: 'https://raw.githubusercontent.com/test-path/builda/latest'
        };
        expect((0, convert_registry_path_to_url_1.convertRegistryPathToUrl)({ registryPath, config, withType: true })).toEqual(expected);
    });
    ...
    test('Should return a path to a registry.json when a custom matcher is provided', () => {
        ...
        expect((0, convert_registry_path_to_url_1.convertRegistryPathToUrl)({ registryPath, config: customConfig })).toEqual(expected);
    });
