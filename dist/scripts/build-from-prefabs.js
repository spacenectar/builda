...
const buildFromPrefabs = (config) => {
    if (config) {
        ...
        if (!prefab) {
            (0, helpers_1.throwError)('No prefab found in config file. Build cannot be run without prefab');
        }
        ...
        node_fs_1.default.readdir(sourceDir, (err, files) => {
            if (err) {
                ...
                files.forEach((file) => {
                    (0, helpers_1.checkAndCopyPath)(node_path_1.default.join(sourceDir, file), destinationDir, file);
                });
            }
        });
    }
