...
const buildFromScaffold = async ({ command, name, options, scaffold }) => {
    ...
    if (config) {
        ...
        const { path, registry, files } = (0, _helpers_1.getModule)('scaffold', scaffold || config.commands[command].use);
        files.forEach((file) => {
