...
describe('convertRegistryPathToUrl() function', () => {
    ...
    test('Should return a path to a registry.json when a custom matcher is provided', () => {
        const customMatcher = {
            original: 'https:\\/\\/(bitbucket.custom.url)\\/projects\\/([\\w\\d-]+){1}\\/repos\\/([\\w\\d-]+){1}\\/browse\\/([\\w\\d-]+)+',
            transformed: 'https://$1/projects/$2/repos/$3/raw/master/$4'
        };
        const registryPath = 'https://bitbucket.custom.url/projects/BULDA/repos/component-library/browse/components/';
        const expected = 'https://bitbucket.custom.url/projects/BULDA/repos/component-library/raw/master/components';
        expect((0, convert_registry_path_to_url_1.convertRegistryPathToUrl)(registryPath, customMatcher)).toEqual(expected);
    });
});
